...
def parse_args():
	...
	args.add_argument("-numnodes", "--numnodes",
					  default=2708, help="num of nodes in graph")
...
def train_model(args):
	...
	if args.is_test:
		model_gat.load_state_dict(torch.load(
		  './checkpoints/{0}/trained_{1}.pth'.format(args.data, args.test_check)))
...
def node_classif(args):
	#print("Performing node classification on cora")
	model_gat = NotGAT(torch.zeros((int(args.numnodes), args.node_out_dim[0])), args.node_out_dim, args.drop_GAT,
					args.alpha, args.nheads_GAT, int(args.numnodes))
	model_gat.cuda()
	model_gat.load_state_dict(torch.load(
	  './checkpoints/{0}/trained_{1}.pth'.format(args.data, args.epochs)))

	y_train, y_val, y_test, indices = load_node_data(args.data)
	#print(len(indices[2]),y_test.shape)
	#print(len(indices[1]),y_val.shape)
	#print(len(indices[0]),y_train.shape)
	#exit()
	r = y_test.shape[0]
	new_test = np.empty([0,y_test.shape[1]])
	for i in range(0,r):
		if sum(y_test[i])>0:
			new_test = np.concatenate((new_test,np.array([y_test[i]])),axis=0)
	...
	for i in range(y_train.shape[0]):
		train_labels.append(np.where(y_train[i] == 1)[0][0])

	for i in range(y_test.shape[0]):
		test_labels.append(np.where(y_test[i] == 1)[0][0])

	train_indices = np.array(indices[0]).astype(np.int32)
	test_indices = np.array(indices[2]).astype(np.int32)
	embeddings = model_gat.final_entity_embeddings.cpu().detach().numpy()
	#print(embeddings.shape)
	#exit()
	clf = LR(penalty='l2', solver='lbfgs', C=1, multi_class='ovr', max_iter=200, n_jobs=20)
	clf.fit(embeddings[train_indices], train_labels)

	preds = clf.predict(embeddings[test_indices])
	#print("accuracy is = ", ((preds == test_labels).sum())/len(test_labels))
	return ((preds == test_labels).sum())/len(test_labels)
acc = node_classif(args)
print(acc)
